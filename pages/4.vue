<template>
    <div
        class="editorial-board min-h-screen bg-[#f4f4f0] text-[#1a1a1a] font-sans selection:bg-[#1a1a1a] selection:text-[#f4f4f0]">
        <div class="max-w-[1400px] mx-auto px-6 md:px-12 py-12">
            ...
            <header class="text-center mb-24">
                <h1 class="text-7xl md:text-9xl font-serif tracking-tighter leading-none mb-6">The Feedback<br><span
                        class="italic font-light text-[#555]">Chronicles</span></h1>
                <div class="w-24 h-[1px] bg-[#1a1a1a] mx-auto mb-8" />
                <p class="max-w-2xl mx-auto text-lg text-[#555] font-serif italic">A curated collection of thoughts,
                    critiques, and visions for the future of our digital ecosystem.</p>

                <button
                    class="mt-12 border border-[#1a1a1a] px-10 py-4 text-xs uppercase tracking-[0.2em] font-semibold hover:bg-[#1a1a1a] hover:text-[#f4f4f0] transition-colors duration-500">
                    Submit a Letter
                </button>
            </header>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-16">
                ...
                <aside class="lg:col-span-3">
                    <div class="sticky top-12">
                        <div class="mb-12">
                            <h2
                                class="text-xs uppercase tracking-[0.2em] font-semibold border-b border-[#1a1a1a] pb-3 mb-6">
                                Index</h2>
                            <ul class="space-y-4">
                                <li v-for="cat in categories" :key="cat"
                                    class="group cursor-pointer flex justify-between items-center">
                ...
                <main class="lg:col-span-9">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-16">
                        <article v-for="(item, index) in feedbacks" :key="item.id" class="group"
                            :class="{ 'md:col-span-2 border-b border-[#1a1a1a] pb-16': index === 0 }">
                            <div class="flex flex-col h-full">
                                <div class="flex justify-between items-start mb-6">
                                    <div class="flex gap-3">
                                        <span
                                            class="text-[10px] uppercase tracking-[0.2em] border border-[#1a1a1a] px-2 py-1">{{
                                            item.status }}</span>
                                        <span class="text-[10px] uppercase tracking-[0.2em] text-[#555] py-1">{{
                                            item.category }}</span>
                                    </div>
                                    <div class="text-xs font-serif italic text-[#888]">{{ item.date }}</div>
                                </div>

                                <h3 class="font-serif leading-tight mb-4 group-hover:text-[#555] transition-colors"
                                    :class="index === 0 ? 'text-5xl' : 'text-3xl'">
                                    {{ item.title }}
                                </h3>

                                <p class="text-[#444] leading-relaxed mb-8 font-serif text-lg"
                                    :class="index === 0 ? 'max-w-3xl' : ''">
                                    {{ item.description }}
                                </p>
                                ...
                                <div class="mt-auto flex items-center justify-between pt-6 border-t border-[#e0e0e0]">
                                    <div class="flex items-center gap-4">
                                        <button class="flex items-center gap-2 group/btn">
                                            <span
                                                class="w-8 h-8 rounded-full border border-[#1a1a1a] flex items-center justify-center group-hover/btn:bg-[#1a1a1a] group-hover/btn:text-[#f4f4f0] transition-colors">
                                        ...
                                        <button
                                            class="text-xs uppercase tracking-[0.1em] text-[#555] hover:text-[#1a1a1a] transition-colors">
                    ...
                    <div class="mt-20 text-center">
                        <button
                            class="text-xs uppercase tracking-[0.2em] font-semibold border-b border-[#1a1a1a] pb-1 hover:text-[#555] hover:border-[#555] transition-colors">
                            Load More Entries
                        </button>
                    </div>
                </main>
