<template>
    <div class="brutalist-board min-h-screen bg-black text-white font-mono selection:bg-[#00ff00] selection:text-black">
        <div class="max-w-6xl mx-auto p-4 md:p-8">
            <!-- Header -->
            <header
                class="border-b-4 border-white pb-8 mb-12 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
                <div>
                    <h1
                        class="text-6xl md:text-8xl font-black uppercase tracking-tighter leading-none hover:text-[#00ff00] transition-colors duration-300">
                        FEEDBACK<br>SYSTEM
                    </h1>
                    <p class="text-xl mt-4 uppercase tracking-widest text-gray-400">Acme Corp // V.1.0.0</p>
                </div>
                <button
                    class="border-2 border-white px-8 py-4 text-xl font-bold uppercase hover:bg-white hover:text-black transition-all duration-200 active:scale-95">
                    + Submit Issue
                </button>
            </header>
                    ...
                    <div class="border-2 border-white p-6">
                        ...
                        <ul class="space-y-4">
                            <li v-for="cat in categories" :key="cat"
                                ...
                                <div
                                    class="w-4 h-4 border-2 border-white group-hover:bg-[#00ff00] group-hover:border-[#00ff00] transition-colors" />
                                <span class="text-lg uppercase group-hover:text-[#00ff00] transition-colors">{{ cat
                                    }}</span>
                            </li>
                        </ul>
                    </div>
                ...
                <main class="lg:col-span-9 space-y-6">
                    <div class="flex justify-between items-center border-b-2 border-white pb-4">
                        ...
                        <div class="flex gap-4 text-sm uppercase">
                            <button
                                class="hover:text-[#00ff00] underline decoration-2 underline-offset-4">Trending</button>
                    ...
                    <div class="space-y-6">
                        <article v-for="item in feedbacks" :key="item.id"
                            class="group border-2 border-white p-6 hover:border-[#00ff00] transition-colors relative overflow-hidden">
                            <div
                                class="absolute top-0 right-0 w-32 h-32 bg-[#00ff00] opacity-0 group-hover:opacity-10 transition-opacity rounded-bl-full" />

                            <div class="flex flex-col md:flex-row gap-6 items-start">
                                <!-- Upvote Block -->
                                <button
                                    class="flex flex-col items-center justify-center border-2 border-white w-20 h-24 group-hover:bg-[#00ff00] group-hover:text-black group-hover:border-[#00ff00] transition-all shrink-0">
                                ...
                                <div class="flex-1">
                                    <div class="flex flex-wrap gap-3 mb-3">
                                        <span class="border border-white px-3 py-1 text-xs uppercase font-bold">{{
                                            item.status }}</span>
                                        <span
                                            class="text-[#00ff00] border border-[#00ff00] px-3 py-1 text-xs uppercase font-bold">{{
                                            item.category }}</span>
                                    </div>
                                    <h3
                                        class="text-3xl font-bold uppercase mb-3 group-hover:text-[#00ff00] transition-colors">
                                        {{ item.title }}</h3>
