...
const state = vi.hoisted(() => ({
  ...
  attachmentRow: {
    storageKey: 'support/final/invoice.pdf',
    fileName: 'invoice.pdf',
    contentType: 'application/pdf',
    conversationId: 'conversation-1',
  },
    ...
    select: vi.fn(() => {
      ...
      const chain: Record<string, any> = {
        ...
        orderBy: vi
          .fn()
          .mockImplementation(() =>
            state.readMode === 'message' && state.selectCount === 2 ? Promise.resolve(state.attachments) : chain
          ),
...
describe('support attachment read routes', () => {
    ...
    expect(message.attachments).toEqual([
      {
        ...
        contentType: 'application/pdf',
        sizeBytes: 10,
        downloadUrl: '/api/support/attachments/attachment-1',
      },
    ])
  ...
  it('serves canonical bytes and uses attachment download hardening', async () => {
    ...
    expect(state.headers['Content-Disposition']).toMatch(
