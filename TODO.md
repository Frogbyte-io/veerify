...
- [ ] **SUP-03-14** Add E2E coverage: inbound mail creates a ticket, a reply threads onto it, a duplicate delivery does not double it
  - **Spec written (`tests/e2e/support-inbound-email.spec.ts`) but NOT YET RUN — deliberately left unchecked.** Covers all three acceptance criteria plus a 401 on unauthenticated delivery, and skips cleanly when `SUPPORT_POSTMARK_WEBHOOK_USER/PASSWORD` are unset (same guard pattern as the Redis/Postgres suites).
  - The blocker was **not** SUP-X-5 (that is fixed — the spec collects). The dev server could not finish a cold start on this machine: individual files were taking 180–320s to compile under contention from the second agent session. `typecheck` and `harness:verify` are green, but that proves it compiles, not that it passes.
  - **To finish:** set the two Postmark vars, start the dev server, and run `PLAYWRIGHT_FORCE=1 PLAYWRIGHT_BASE_URL=http://localhost:<port> npx playwright test tests/e2e/support-inbound-email.spec.ts`. Check it off only once it actually passes.
