...
- [ ] **SUP-00-10** Add `GET /api/system/tls-ask` — the Caddy on-demand TLS validation endpoint
  - `Caddyfile` points `on_demand_tls.ask` at `http://app:3000/api/system/tls-ask`, which does not exist yet. Caddy treats a non-2xx ask response as "deny", so this **fails closed**: no certificate is issued for any host, and custom domains will not serve HTTPS at all until this lands. Not a security hole — a functionality blocker.
...
- [ ] **SUP-00-9** Migrate `NotificationBell.vue` off direct WS payloads onto the channel system: publish a thin envelope on `user:<id>` and have the client refetch the list and unread count instead of unshifting `msg.data`
  - **Server half done** in `b6404a9`: the envelope now requires a scope id matching its channel rather than a `teamId`, so `user:<id>` events are expressible. `publishRealtime` also refuses cross-channel publishes now, which is a stronger guarantee than Stage 00 originally specified.
  - **Remaining:** switch `sendToUser` to publish `{type:'notification.created', userId}` on `user:<id>`, and change `NotificationBell.vue`'s `onmessage` to refetch the list and unread count instead of unshifting `msg.data`. Only then may the 30s polling fallback be reconsidered.
