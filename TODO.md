...
- [x] **SUP-00-7** Add `Dockerfile` + `.dockerignore`; extend production `docker-compose.yml` with `app`, `valkey`, `minio`, and Caddy on-demand TLS; add `valkey` to dev compose; document the VM path in `README.md`
  ...
  - `docker build` reported successful by the implementing agent (exit 0, image `109cfe5e98e6`, 1.76 GB; `docker inspect` confirmed non-root `nuxt` user, entrypoint, and healthcheck). Its first report said the daemon was unavailable; the daemon came online ~40 min later and it corrected itself. An independent orchestrator rebuild is in progress to confirm.
