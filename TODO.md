...
- [x] **SUP-00-7** Add `Dockerfile` + `.dockerignore`; extend production `docker-compose.yml` with `app`, `valkey`, `minio`, and Caddy on-demand TLS; add `valkey` to dev compose; document the VM path in `README.md`
  ...
  - **`docker build` is UNVERIFIED.** The agent could not start the Docker daemon, and the orchestrator did not attempt it either because the `D:` drive was at 100% (65 MB free) and a build could have destabilised the machine. Run `docker build .` after freeing space before trusting this.
