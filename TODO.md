...
- [ ] **SUP-X-5** (repo-wide) Fix E2E specs that import `db` failing to collect under Playwright (delta D-33). `server/utils/logger.ts` imports `createConsola` from `consola`; Playwright resolves consola's browser export condition, which does not provide it, so collection dies with `No tests found`. Pre-existing — `support-contact-timeline.spec.ts` from Stage 01 fails identically. **First establish whether these specs pass in CI**: the E2E gate _skips_ rather than fails when its guard is unsatisfied, so `yarn harness:verify` reports green either way, and several Stage 01/02 acceptance criteria worded "verified by E2E" may not be enforced anywhere.
