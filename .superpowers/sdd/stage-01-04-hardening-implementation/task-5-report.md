...
- GREEN: the focused real-Postgres suite passes 18/18. It uses backend PIDs, `pg_blocking_pids()`, and `pg_locks` rather than timing guesses; covers both merge acquisition orders, block/delete/new-ambiguity, policy-disable linearization, exact-one/zero/blocked/merged/ambiguous/cross-team/email-only/anonymous, duplicate callers, preservation on disable, and unlink.
...
- Postgres integration: 7 files / 60 passed.
